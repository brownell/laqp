...
from processor import _score_qsos
...
def cross_check_all_logs(year=CONTEST_YEAR):
    ...
    print("\nRecalculating final scores...")
    for result in all_results:
        recalculate_final_score(result)
    
    # Save updated results
    print("Saving updated results to database...")
    from database import save_contest_result
    for result in all_results:
        save_contest_result(result)
...
def recalculate_final_score(result):
    ...
    This calls the existing _score_qsos() function from processor.py,
