#Use this script to score each of the prepared logs in the input directory (PreparedLogs)
...
NonTexasCats = {"DX SO", "NTX SO QRP", "NTX SO", "NTX SO PHO", "NTX SO CWO"}
...
#-----------------------------------------------------------------------------------------
### use the following header for final output
print("LogFile,Call,Email,Category,Club,Operator,Claimed Score,CWQs,PHQs,DGQs,QSOPts,Mults,Score*,MTB,CAB,TotalBonus,TotalScore,ScoreReduction")

#----------------------------------------------------------------------------------
# The following section reads each log and builds a list of mobile calls (TheMobileCalls)
# By finding stations in which the TQP-CATEGORY contains "TXM"
#--------------------------------------------------------------------------------------
TheMobileCalls = []
TheMobileCalls.clear()
...
for f in all_files:
    ...
    with open(f, 'r', encoding='utf-8') as file:
        ...
        for line in file:
            ...
            if(len(items) <= 0): 
                continue
            if(items[0] == "TQP-CATEGORY:" and "TXM" in processed_line):
                haveAMobile = True
            if(items[0] == "CALLSIGN:"):
                callSignSplit = items[1].split("/")
                CallSign = callSignSplit[0]
                haveACall = True
        if(haveACall and haveAMobile):
           TheMobileCalls.append(CallSign)
           continue
    ...
    with open(f, 'r', encoding='utf-8') as file:
        ...
        for line in file:
            ...
            if(items[0] == "CLUB:"):
                for i in range(1, len(items)):
                    Club = Club + items[i] + " "
            if(items[0] == "OPERATORS:"):
                for i in range(1, len(items)):
                    Operators = Operators + items[i] + " "
                Operators = Operators.replace(',', '')
            if(items[0] == "CLAIMED-SCORE:"):
                if(len(items) > 1):
                    ClaimedScore = items[1]
                    ClaimedScore = ClaimedScore.replace(',', '') 
                else:
                    ClaimedScore = "0"
            if(TQPCat.rstrip() in NonTexasCats):
                Club = ""
            if(items[0] == "QSO:"):
                rcvdCall = items[8].upper().split("/")
                sentQth = items[7].upper()
                sentCall = items[5].upper() 
                rcvdQth = items[10].upper()
                band = items[1]
                mode = items[2].upper()
                dupeLine = rcvdCall[0] + "_" + band + "_" + mode + "_" + sentCall + "_" + sentQth + "_" + rcvdQth
                if dupeLine not in dupeList:
                    dupeList.append(dupeLine)
                else: 
                    removedDupesList.append(dupeLine)
        ...
        for dupe in dupeList:
            dupeSplit = dupe.split("_")
            if len(dupeSplit) < 6:
                continue
            rQth = dupeSplit[5].rstrip()
            mode = dupeSplit[2]
            if mode == "CW":
               CWQs += 1
            if mode == "PH":
               PHQs += 1
            if mode == "DG" or mode == "RY":
                DGQs += 1
            if rQth not in multList:
               multList.append(rQth)
            aSentCall = dupeSplit[3]
            aSentQth = dupeSplit[4]
            aRcvdCall = dupeSplit[0]
            if(aSentQth in TXabbList):
                ctysSent.append(aSentQth)
                if(aSentQth not in uniqueCtysSent):
                    uniqueCtysSent.append(aSentQth)
            if(aRcvdCall in TheMobileCalls):
                ctysRcvd.append(aRcvdCall + "_" + rQth) #the current station logged a QSO with mobile(aRcvdCall) in cty rQth
                if(aRcvdCall not in uniqueCallsRcvd):
                    uniqueCallsRcvd.append(aRcvdCall)  #uniqueCallsRcvd is a list of callsigns of mobiles worked 
        ...
        for ctys in uniqueCtysSent:
            ...
            if numQ >= 5:
                ...
                print("CountyActivation for, " + aSentCall + "," + ctys)
                
        if(len(uniqueCtysSent) <= 2):
            CountyActivationBonus = 0

#        for item in ctysRcvd:
#            print(sentCall + "," + item) 

        MobileTrackingBonus = 0
        for calls in uniqueCallsRcvd:
            numCtysWorkedMobile = 0
    ...
    print(fname + "," + CallSign + "," + email.lower() + "," + TQPCat + "," + Club + "," + Operators + "," + ClaimedScore + "," + str(CWQs) + "," + str(PHQs) + "," + str(DGQs) + "," + str(QsoPts) + "," + str(Mults) + "," + str(ScoreWOBonus) + "," + str(MobileTrackingBonus) + "," + str(CountyActivationBonus) + "," + str(TotalBonus) + "," + str(TotalScore) + "," + str(ScoreReduction))
